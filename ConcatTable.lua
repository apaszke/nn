...
function ConcatTable:updateOutput(input)
   for i=1,#self.modules do
      self.output[i] = self:callAndRethrow(self.modules[i], i, 'updateOutput', input)
   end
   return self.output
end
...
local function backward(self, method, input, gradOutput, scale)
   ...
   if isTable then
      for i,module in ipairs(self.modules) do
         local currentGradInput = self:callAndRethrow(module, i, method, input, gradOutput[i], scale)
...
function ConcatTable:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   for i,module in ipairs(self.modules) do
      self:callAndRethrow(module, i, 'accGradParameters', input, gradOutput[i], scale)
   end
end

function ConcatTable:accUpdateGradParameters(input, gradOutput, lr)
   for i,module in ipairs(self.modules) do
      self:callAndRethrow(module, i, 'accUpdateGradParameters', input, gradOutput[i], lr)
   end
end
