...
function Concat:updateOutput(input)
   ...
   for i=1,#self.modules do
      local currentOutput = self:callAndRethrow(self.modules[i], i, 'updateOutput', input)
...
function Concat:updateGradInput(input, gradOutput)
   ...
   for i,module in ipairs(self.modules) do
      ...
      local currentGradInput = self:callAndRethrow(module, i, 'updateGradInput', gradOutput:narrow(self.dimension, offset, currentOutput:size(self.dimension)))
...
function Concat:accGradParameters(input, gradOutput, scale)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      self:callAndRethrow(module, i, 'accGradParameters',
          input,
...
function Concat:backward(input, gradOutput, scale)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      local currentGradInput = self:callAndRethrow(module, i, 'backward', input, gradOutput:narrow(self.dimension, offset, currentOutput:size(self.dimension)), scale)
      if currentGradInput then -- if the module does not produce a gradInput (for example first layer), then ignore it and move on.
         if i==1 then
            self.gradInput:copy(currentGradInput)
         else
...
function Concat:accUpdateGradParameters(input, gradOutput, lr)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      self:callAndRethrow(module, i, 'accUpdateGradParameters',
          input,
          gradOutput:narrow(self.dimension, offset, currentOutput:size(self.dimension)),
          lr)
      offset = offset + currentOutput:size(self.dimension)
