...
function ParallelTable:updateOutput(input)
   for i=1,#self.modules do
      self.output[i] = self:callAndRethrow(self.modules[i], i, 'updateOutput', input[i])
   end
   return self.output
end
...
function ParallelTable:updateGradInput(input, gradOutput)
   for i,module in ipairs(self.modules) do
      self.gradInput[i] = self:callAndRethrow(module, i, 'updateGradInput', input[i], gradOutput[i])
   end
...
function ParallelTable:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   for i,module in ipairs(self.modules) do
      self:callAndRethrow(module, i, 'accGradParameters', input[i], gradOutput[i], scale)
   end
end
...
function ParallelTable:accUpdateGradParameters(input, gradOutput, lr)
   lr = lr or 1
   for i,module in ipairs(self.modules) do
      self:callAndRethrow(module, i, 'accUpdateGradParameters', input[i], gradOutput[i], lr)
   end
end
