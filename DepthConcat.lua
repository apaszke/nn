...
function DepthConcat:updateGradInput(input, gradOutput)
   ...
   for i,module in ipairs(self.modules) do
      ...
      local currentGradInput = self:callAndRethrow(module, i, 'updateGradInput', input, gradOutputWindow)
      if i==1 then
...
function DepthConcat:accGradParameters(input, gradOutput, scale)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      local gradOutputWindow = self:windowNarrow(gradOutput, currentOutput, offset)
      self:callAndRethrow(module, i, 'accGradParameters', input, gradOutputWindow, scale)
      offset = offset + currentOutput:size(self.dimension)
   end
end
...
function DepthConcat:backward(input, gradOutput, scale)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      local gradOutputWindow = self:windowNarrow(gradOutput, currentOutput, offset)
      local currentGradInput = self:callAndRethrow(module, i, 'backward', input, gradOutputWindow)
      if i==1 then
         self.gradInput:copy(currentGradInput)
      else
         self.gradInput:add(currentGradInput)
...
function DepthConcat:accUpdateGradParameters(input, gradOutput, lr)
   ...
   for i,module in ipairs(self.modules) do
      local currentOutput = module.output
      local gradOutputWindow = self:windowNarrow(gradOutput, currentOutput, offset)
      self:callAndRethrow(module, i, 'accUpdateGradParameters', input, gradOutputWindow, lr)
      offset = offset + currentOutput:size(self.dimension)
   end
end
